...
def generar_reporte_dashboard_html(df_cli: pd.DataFrame) -> str:
    """Genera un reporte HTML completo del dashboard para descargar"""
    ...
    if not df_analisis.empty:
        ...
        total_cartera = total_monto_esperado = total_retorno = prom_riesgo = prom_conversion = 0
    ...
    <head>
        <meta charset="UTF-8">
        <title>Reporte Dashboard - Kapitaliza CRM</title>
        <style>
            @media print {{
    ...
    <body>
        <div class="header">
            <h1>📊 Reporte Dashboard - CRM Kapitaliza</h1>
        ...
        <div class="section">
            ...
            <div class="kpi-grid">
                ...
                <div class="kpi-card">
                    <div class="kpi-label">✅ Dispersados (Éxito)</div>
                    <div class="kpi-value">{dispersados}</div>
                    <div class="kpi-delta">{tasa_exito:.1f}% del total</div>
                </div>
                <div class="kpi-card">
                    <div class="kpi-label">⏳ En Proceso</div>
                    <div class="kpi-value">{en_proceso}</div>
                    <div class="kpi-delta">{tasa_proceso:.1f}% del total</div>
                </div>
                <div class="kpi-card">
                    <div class="kpi-label">❌ Rechazados</div>
                    <div class="kpi-value">{rechazados}</div>
                    <div class="kpi-delta">{tasa_rechazo:.1f}% del total</div>
                </div>
            </div>
        </div>
        
        <div class="section">
            <div class="section-title">💹 Top Estatus por Monto</div>
            <p><strong>Total Presupuesto General: {formatear_monto(total_presupuesto)}</strong></p>
            <div class="top-estatus-grid">
    """
    
    # Agregar tarjetas de top estatus
    for estatus, data in top_estatus.iterrows():
        ...
        html += f"""
                <div class="estatus-card">
                    <div class="estatus-name">{estatus}</div>
                    <div class="estatus-monto">{formatear_monto(monto_total)}</div>
                    <div class="estatus-detail">{int(cantidad)} clientes • Promedio: {formatear_monto(promedio)}</div>
                </div>
        """
    ...
    if total_cartera > 0:
        html += f"""
                    <li><strong>Cartera total:</strong> {formatear_monto(total_cartera)}</li>
                    <li><strong>Conversión esperada:</strong> {formatear_monto(total_monto_esperado)} ({(total_monto_esperado/total_cartera*100):.1f}% de la cartera)</li>
                    <li><strong>Retorno esperado:</strong> {formatear_monto(total_retorno)}</li>
                    <li><strong>Riesgo promedio:</strong> {prom_riesgo:.1f}%</li>
                    <li><strong>Conversión media:</strong> {prom_conversion:.1f}%</li>
        """
    else:
        html += """
                    <li>No hay datos con montos registrados para análisis financiero</li>
        """
    ...
    html += """
        ...
        <div class="footer">
            <p>Generado por CRM Kapitaliza • Este reporte es confidencial</p>
        </div>
    </body>
    </html>
    """
